...
#define GPIOx_MODE_R_OFFSET  (uint32_t*)        0x00
//#define GPIOx_OD_R_OFFSET    (uint32_t*)        0x14
#define GPIOx_PUPDR_OFFSET   (uint32_t*)        0x0C

#define GPIOx_MODE_R  (uint32_t*)(GPIOx + GPIOx_MODE_R_OFFSET) 
//#define GPIOx_OD_R    (uint32_t*)(GPIOx + GPIOx_OD_R_OFFSET) 
#define GPIOx_PUPDR   (uint32_t*) (GPIOx +  GPIOx_PUPDR_OFFSET)

 

#define INPUT                            (uint8_t) 0x0
#define OUTPUT                           (uint8_t) 0x1
#define ALTERNATE                        (uint8_t) 0x2 //I2C SPI UART
#define ANALOG_INPUT                     (uint8_t) 0x3

#define OUTPUT_PUSH_PULL                 (uint8_t) 0x0
#define OUTPUT_OPEN_DRAIN                (uint8_t) 0x1

void GPIO_Clock_Enable(unsigned int *GPIOx);
